      ...
      (
        ...
        <Accordion.Content className="bg-white">
          ...
          .map(
           ...
           {
                enrollmentData?.quizzesWithQuestions?.length > 0 &&
                enrollmentData?.quizzesWithQuestions
                  ?.filter((quiz: any) => quiz.module === _id)
                  .map((quiz: any, index: any) => (
                    <Link
                      href={"/dashboard/quiz/[courseId]/[quiz]"}
                      as={`/dashboard/quiz/${enrollmentData?.course?._id}/${quiz?.quiz?._id}`}
                      key={index}
                    >
